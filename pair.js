...
// ---------------- CONFIG ----------------
const BOT_NAME_FANCY = '𝐊𝐘𝐙𝐄𝐑-𝐌𝙳 4.0.0𝗩 🥷🇱🇰';
...
const config = {
  ...
  RCD_IMAGE_PATH: 'https://files.catbox.moe/ut2xyq.png',
  NEWSLETTER_JID: 'jid',
  OTP_EXPIRY: 300000,
  WORK_TYPE: 'public',
  OWNER_NUMBER: process.env.OWNER_NUMBER || '94704290562',
  CHANNEL_LINK: 'https://chat.whatsapp.com/HRIlSELlxO5JQo2gYL4MzV?mode=gi_t',
  BOT_NAME: '𝐊𝐘𝐙𝐄𝐑-𝐌𝙳 4.0.0𝗩 🥷🇱🇰',
  BOT_VERSION: '4.0.0V',
  OWNER_NAME: '𝐊𝐚𝐬𝐮𝐧 𝐁𝐡𝐚𝐠𝐲𝐚 🥷🇱🇰',
  IMAGE_PATH: 'https://files.catbox.moe/ut2xyq.png',
  BOT_FOOTER: '> *〠 𝐏𝙾𝚆𝙴𝚁𝙴𝙳 𝗕𝗬 𝐊𝐘𝐙𝐄𝐑-𝐌𝙳 𝐕.4 🥷🇱🇰*',
  BUTTON_IMAGES: { ALIVE: 'https://files.catbox.moe/ut2xyq.png' }
};
// ---------------- MONGO SETUP ----------------

const MONGO_URI = process.env.MONGO_URI || 'mongodb+srv://dtz_dula:dtz_nova_x_md@cluster0.evuexjr.mongodb.net/?appName=Cluster0';
const MONGO_DB = process.env.MONGO_DB || 'KASUN_BHAGYA_DATE';
...
`;
        ...
        const buttonMessage = {
            ...
            footer: `${config.BOT_FOOTER || config.BOT_NAME}`,
            buttons: buttons,
            headerType: 4,
            contextInfo: {
                externalAdReply: {
...
case 'autolikestatus': {
  try {
    ...
    if (option === 'on') {
      cfg.AUTO_LIKE_STATUS = 'true';
      await setUserConfigInMongo(sanitized, cfg);
      await socket.sendMessage(sender, { text: '✅ *Auto Like Status:* ON' }, { quoted: msg });
    } else if (option === 'off') {
      cfg.AUTO_LIKE_STATUS = 'false';
      await setUserConfigInMongo(sanitized, cfg);
      await socket.sendMessage(sender, { text: '❌ *Auto Like Status:* OFF' }, { quoted: msg });
    } else {
      await socket.sendMessage(sender, { text: `ℹ️ *Auto Like Status*\n\nUsage:\n${config.PREFIX}autolikestatus on\n${config.PREFIX}autolikestatus off` }, { quoted: msg });
    }
  } catch (e) {
    console.error('Autolikestatus command error:', e);
  }
  break;
}
...
> *〠 𝐏𝙾𝚆𝙴𝚁𝙴𝙳 𝗕𝗬 𝐀𝚂𝙷𝙸𝚈𝙰-𝐌𝙳 𝐕.3 🥷🇱🇰*`,
            contextInfo: {
                externalAdReply: {
                    ...
                    thumbnailUrl: result.thumbnail || "https://files.catbox.moe/ut2xyq.png",
                    sourceUrl: url,
                    mediaType: 1,
                    renderLargerThumbnail: true
                }
...
[⬜⬜⬜⬜⬜⬜⬜⬜⬜⬜] 0%`, 
    ...
    const defaultLogo = "https://files.catbox.moe/ut2xyq.png";
...
case 'menu': {
    try {       
        ...
        const MENU_IMG = "https://files.catbox.moe/ut2xyq.png"; 
        // 👇 Video Note URL
...
case 'download': {
  ...
  try {
    ...
    const logos = [
        "https://files.catbox.moe/ut2xyq.png", 
        "https://files.catbox.moe/ut2xyq.png",
...
case 'creative': {
  ...
  try {
    ...
    // Random Logo Logic
    const logos = [config.LOGO, "https://files.catbox.moe/ut2xyq.png"]; // Add more
    const randomLogo = logos[Math.floor(Math.random() * logos.length)] || logos[0];
...
case 'system': {
  ...
  try {
    ...
    // Random Logo Logic
    const logos = [config.LOGO, "https://files.catbox.moe/ut2xyq.png"]; 
    const randomLogo = logos[Math.floor(Math.random() * logos.length)] || logos[0];
...
case 'dp': {
    ...
    try {
        ...
        const logo = cfg.logo || "https://files.catbox.moe/ut2xyq.png"; // Default Logo
...
case 'kyzer': {
  try {
    ...
    const ownerNumber = '94704290562';
    const ownerName = '𝐀ʏᴇꜱʜ 𝐓ʜᴇᴍɪʏᴀ 🥷🇱🇰';
    const botName = '𝐀𝚂𝙷𝙸𝚈𝙰-𝐌𝙳 4.0.0𝗩 🥷🇱🇰';
    const ownerImage = 'https://files.catbox.moe/ut2xyq.png';
    const websiteUrl = 'https://kasun_bhagya-ofc-site.vercel.app/';
...
👇 *ꜱᴇʟᴇᴄᴛ ʏᴏᴜʀ ᴅᴏᴡɴʟᴏᴀᴅ ᴛʏᴘᴇ* 👇`;
        ...
        const buttonMessage = {
            image: { url: result.cover || result.thumbnail || "https://files.catbox.moe/ut2xyq.png" },
            caption: captionMessage,
            footer: `〠 𝐏𝙾𝚆𝙴𝚁𝙴𝙳 𝗕𝗬 𝐀𝚂𝙷𝙸𝚈𝙰-𝐌𝙳 𝐕.4 🥷🇱🇰`,
            buttons: buttons,
            headerType: 4,
...
case 'tagall': {
  try {
    ...
    let groupPP = 'https://files.catbox.moe/ut2xyq.png';
...
case 'gstatus': {
  try {
    ...
    const botName = cfg.botName || BOT_NAME_FANCY;
    const groupName = gm.subject || 'Group';
    const totalMembers = (gm.participants || []).length;
    const admins = (gm.participants || []).filter(p => p.admin).length;
    let groupPP = 'https://files.catbox.moe/ut2xyq.png';
    try { groupPP = await socket.profilePictureUrl(from, 'image'); } catch(e){}
    const metaQuote = {
      key: { remoteJid: "status@broadcast", participant: "0@s.whatsapp.net", fromMe: false, id: "META_AI_GSTATUS" },
      message: { contactMessage: { displayName: botName, vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${botName};;;;\nFN:${botName}\nORG:Meta Platforms\nTEL;type=CELL;type=VOICE;waid=13135550002:+1 313 555 0002\nEND:VCARD` } }
    };
    let caption = `╔══『 👥 *𝐆𝐑𝐎𝐔𝐏 𝐒𝐓𝐀𝐓𝐔𝐒* 』═══❒\n`;
    caption += `╠⦁ 📌 *𝐆roup:* ${groupName}\n`;
    caption += `╠⦁ 👥 *𝐌embers:* ${totalMembers}\n`;
    caption += `╠⦁ 👑 *𝐀dmins:* ${admins}\n`;
    caption += `╠⦁ 🆔 *𝐉ID:* ${from}\n`;
    caption += `╚═════════════════════════❒\n`;
    caption += `\n━━━━━━⊱ *${botName}* ⊰━━━━━━`;
    await socket.sendMessage(from, { image: { url: groupPP }, caption }, { quoted: metaQuote });
  } catch (err) {
    console.error('gstatus error', err);
    await socket.sendMessage(sender, { text: '❌ Error running gstatus.' }, { quoted: msg });
  }
  break;
}
...
case 'setppfull': {
  try {
    ...
    const quotedMsg = ctxInfo.quotedMessage;
    const media = await downloadQuotedMedia(quotedMsg).catch(() => null);
    if (!media) return await socket.sendMessage(sender, { text: '❌ Please reply to an image to set as full profile picture.' }, { quoted: msg });
    await socket.updateProfilePicture(socket.user.id, media);
    await socket.sendMessage(sender, { text: '✅ *Bot profile picture updated successfully!*' }, { quoted: msg });
  } catch (e) {
    console.error('Setppfull command error:', e);
...
case 'creact': {
  ...
  // Format: .creact postLink,reactCount,emoji1 emoji2 emoji3...
  if (!q.includes(',')) {
    return await socket.sendMessage(sender, { 
      text: "❌ Usage: .creact <postLink>,<reactCount>,<emoji1> <emoji2>...\n\n*උදාහරණ:*\n.creact channelId/messageId,1K,❤️ 😂 🔥\n.creact channelId/messageId,2000,❤️ 👍" 
    }, { quoted: msg });
  }
  ...
  try {
    ...
    let postLink = parts[0].trim();
    let reactCountStr = parts[1].trim().toUpperCase();
    const emojisStr = parts[2].trim();

    // Parse react count (support K, M notations)
    let reactCount = 0;
    if (reactCountStr.includes('K')) {
      reactCount = parseInt(reactCountStr.replace('K', '')) * 1000;
    } else if (reactCountStr.includes('M')) {
      reactCount = parseInt(reactCountStr.replace('M', '')) * 1000000;
    } else {
      reactCount = parseInt(reactCountStr);
    }

    if (isNaN(reactCount) || reactCount < 1) {
      return await socket.sendMessage(sender, { 
        text: "❌ React count must be a number greater than 0! (1K, 2K, 3K, 1000, 2000, etc.)" 
      }, { quoted: msg });
    }

    // Maximum limit to prevent abuse (10K per session)
    const MAX_REACTIONS = 10000;
    if (reactCount > MAX_REACTIONS) {
      return await socket.sendMessage(sender, { 
        text: `❌ Maximum reaction limit is ${MAX_REACTIONS}! (Current: ${reactCount})` 
      }, { quoted: msg });
    }

    // Parse post link to extract channelJid and messageId
    let channelJid = null;
    let messageId = null;

    if (postLink.includes('/')) {
      const urlParts = postLink.split('/');
      if (urlParts.some(p => p.includes('channel'))) {
        const channelIndex = urlParts.findIndex(p => p.includes('channel'));
        if (channelIndex >= 0 && channelIndex + 1 < urlParts.length) {
          channelJid = urlParts[channelIndex + 1];
          if (!channelJid.endsWith('@newsletter')) {
            channelJid = `${channelJid}@newsletter`;
          }
        }
        const messageIndex = urlParts.findIndex(p => p.includes('message'));
        if (messageIndex >= 0 && messageIndex + 1 < urlParts.length) {
          messageId = urlParts[messageIndex + 1];
        }
      }
    } else {
      const simpleParts = postLink.split('/');
      if (simpleParts.length >= 2) {
        channelJid = simpleParts[0];
        messageId = simpleParts[1];
        if (!channelJid.endsWith('@newsletter')) {
          channelJid = `${channelJid}@newsletter`;
        }
      }
    ...
    if (!channelJid || !messageId) {
      return await socket.sendMessage(sender, { 
        text: "❌ Invalid post link format! Use: channelId/messageId" 
      }, { quoted: msg });
    }

    // Parse emojis from the emoji string
    const emojis = emojisStr.split(/\s+/).filter(e => e.trim());
    ...
    // Validate emojis are actual emoji characters
    const validEmojis = emojis.filter(e => /\p{Emoji}/u.test(e));
    if (validEmojis.length === 0) {
      return await socket.sendMessage(sender, { 
        text: "❌ Please provide valid emoji characters!" 
      }, { quoted: msg });
    }

    const totalReactions = reactCount * validEmojis.length;
    const estimatedTime = Math.ceil((totalReactions * 1000) / 60); // Rough estimate in seconds

    await socket.sendMessage(sender, { 
      text: `⏳ *Starting Channel Reactions*\n\n🎯 Total Reactions: ${totalReactions.toLocaleString()}\n📝 Emojis: ${validEmojis.join(' ')}\n⏱️ Estimated Time: ~${estimatedTime}s\n\n*මෙතුරු brett පුරුවන්න...*` 
    }, { quoted: msg });

    let successCount = 0;
    let failedCount = 0;
    let updateCounter = 0;
    const updateInterval = Math.max(50, Math.floor(totalReactions / 10)); // Update 10 times during process

    // React with each emoji for the specified count
    for (let i = 0; i < reactCount; i++) {
      for (const emoji of validEmojis) {
        try {
          await socket.newsletterReactMessage(channelJid, messageId.toString(), emoji);
          await saveNewsletterReaction(channelJid, messageId.toString(), emoji, sanitized);
          successCount++;
          updateCounter++;

          // Send progress update every N reactions
          if (updateCounter % updateInterval === 0) {
            const progress = Math.round((successCount / totalReactions) * 100);
            try {
              await socket.sendMessage(sender, { 
                text: `⏳ Progress: ${progress}% (${successCount}/${totalReactions})` 
              }, { quoted: msg });
            } catch(e) {}
          }

          // Dynamic delay - faster for large batches
          const delayMs = reactCount > 1000 ? 300 : reactCount > 500 ? 500 : 800;
          await delay(delayMs);
        } catch (e) {
          console.error(`Failed to react with ${emoji}:`, e);
          failedCount++;
          // Slightly longer delay on error
          await delay(1000);
        }
      }
    }

    // Send final success message with image
    const metaQuote = {
      key: { remoteJid: "status@broadcast", participant: "0@s.whatsapp.net", fromMe: false, id: "META_AI_CREACT" },
      message: { contactMessage: { displayName: botName, vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${botName};;;;\nFN:${botName}\nORG:Meta Platforms\nTEL;type=CELL;type=VOICE;waid=13135550002:+1 313 555 0002\nEND:VCARD` } }
    };

    let imagePayload = String(logo).startsWith('http') ? { url: logo } : fs.readFileSync(logo);

    const successRate = totalReactions > 0 ? Math.round((successCount / totalReactions) * 100) : 0;
    const resultText = `✅ *Channel Reaction Complete!*\n\n🎯 *Channel:* ${channelJid}\n📨 *Message ID:* ${messageId}\n😊 *Emojis:* ${validEmojis.join(' ')}\n🔢 *Reaction Count:* ${reactCount.toLocaleString()}\n\n✔️ *Total Success:* ${successCount.toLocaleString()}\n❌ *Total Failed:* ${failedCount.toLocaleString()}\n📊 *Success Rate:* ${successRate}%\n\n🕐 Completed By: @${senderIdSimple}`;

    await socket.sendMessage(sender, {
      image: imagePayload,
      caption: resultText,
      footer: `🍁 ${botName} CHANNEL REACT`,
      mentions: [nowsender],
      buttons: [{ buttonId: `${config.PREFIX}menu`, buttonText: { displayText: "📄 𝘔𝘦𝘯𝘶" }, type: 1 }],
      headerType: 4
    }, { quoted: metaQuote });
